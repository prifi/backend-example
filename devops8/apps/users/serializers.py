...
class UserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        mail.delay("通知", validated_data['username'], settings.EMAIL_FROM, ['787696331@qq.com'],)

        # 同步创建gitlab账户
        res = gl.users.create({'username': validated_data['username'], 'password':validated_data['password'],
                              'email': instance.email, 'name': validated_data['name']})
        print(res)
        instance.save()
        return instance

    # 如果更新时候 需要修改密码，则需要对密码单独处理，故而需要重写下
